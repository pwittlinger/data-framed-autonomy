(define (domain trace-alignment)
  ...
  (:types activity automaton_state trace_state parameter_name value_name constraint job_id)
  ...
  (:predicates 
    ...
    (cur_s_state ?s - automaton_state ?j - job_id)
    (goal_state ?s - automaton_state)
    (final_t_state ?t - trace_state)
    (violated ?c - constraint ?j - job_id)
    (initial_state ?s1 - automaton_state) 
    (trace_associated ?t - trace_state ?j - job_id)
    ...
    (invalid ?s1 - automaton_state ?a - activity ?s2 - automaton_state ?j - job_id)
  ...
  ;; --------------------------------------------------------------------------------------------------
    (:action add_action
    :parameters (?a - activity ?j - job_id)
    :precondition (and 
      (not (after_add))
      (not (complete_sync ?a))
      (not (after_sync))
  ...
  (:action add_parameter
      :parameters (?a - activity ?s1 - automaton_state ?pn - parameter_name ?vn - value_name ?j - job_id)
      :precondition (and 
      (complete_sync ?a)
      (after_add)
    ...
    :effect (and 
      ...
      (forall (?s1 - automaton_state ?s2 - automaton_state ?c - constraint)
            (when (and
                ...
                (or
                ;(> (- (+(current_timestamp)0.1) (start_clock ?c ?j) ) (max_t_condition ?s1 ?a ?s2))
                ;(< (- (+(current_timestamp)0.1) (start_clock ?c ?j) ) (min_t_condition ?s1 ?a ?s2))
                (< (current_timestamp) (+ (min_t_condition ?s1 ?a ?s2) (start_clock ?c ?j) ))
                (> (current_timestamp) (+ (max_t_condition ?s1 ?a ?s2) (start_clock ?c ?j) )) 
                ;(> (- (current_timestamp) (start_clock ?c ?j)) (max_t_condition ?s1 ?a ?s2))
                ;(< (- (current_timestamp) (start_clock ?c ?j)) (min_t_condition ?s1 ?a ?s2))
                )
                ) 
  ...
  (:action add_move_automata
      ...
      :effect (and 
        ...
        (when (and
          ...
          ;(<= (+(current_timestamp)0.1) (+ (max_t_condition ?s1 ?c ?s2) (start_clock ?c ?j)))
          ;(>= (+(current_timestamp)0.1) (+ (min_t_condition ?s1 ?c ?s2) (start_clock ?c ?j)))
          ...
          ;(assign (start_clock ?c ?j) (+(current_timestamp)0.1))
          (assign (start_clock ?c ?j) (current_timestamp))
        ))
      )
  ...
  (:action validate-payload
      :parameters (?t1 - trace_state ?a - activity ?t2 - trace_state ?j - job_id)
      :precondition (and 
        (cur_t_state ?t1) 
        (trace_associated ?t1 ?j)
        (trace_associated ?t2 ?j)
        (trace ?t1 ?a ?t2) 
        (not (after_sync))
        (not (after_add))
        (not (failure))
  ...
  (:action sync-actions
    :parameters (?t1 ?t2 - trace_state ?a - activity ?j - job_id)
    :precondition (and 
      (trace_associated ?t1 ?j)
      (trace_associated ?t2 ?j)
    ...
    :effect (and 
      ...
      (forall (?s1 - automaton_state ?s2 - automaton_state ?c - constraint)
        (when (and
          ...
          ;(<= (timestamp ?t1 ?t2) (+ (start_clock ?c ?j) (max_t_condition ?s1 ?c ?s2)))
          ;(>= (timestamp ?t1 ?t2) (+ (start_clock ?c ?j) (min_t_condition ?s1 ?c ?s2)))
  ...
  (:process waiting
        ...
        :precondition (and
                ...
                (not (after_add_check))
                (or
                (exists (?s1 ?s2 - automaton_state ?a - activity ?n - constraint ?j - job_id) 
                    (and
                        (cur_s_state ?s1 ?j)
                        (associated ?s1 ?n)
                        (associated ?s2 ?n)
