(define (domain trace-alignment)
  ...
  (:action check_added_parameter_model
    ...
    :effect (and 
      ...
      (forall (?pn - parameter_name ?s1 - automaton_state ?s2 - automaton_state ?c - constraint)
  ...
  ;; VALIDATING FAILURE AND RESETTING
  ;

  (:action validate-failure-after-recovery
      :parameters (?c - constraint ?a - activity)
      :precondition (and 
  ...
  (:action set-failure
      ...
      :precondition (and
          ...
          (not (exists (?s1 - automaton_state ?a - activity ?s2 - automaton_state) 
            (and 
              (automaton ?s1 ?a ?s2)
              (cur_s_state ?s1)
              (associated ?s1 ?c)
...
(:process waiting_add
        :parameters (?s1 ?s2 - automaton_state ?a - activity ?n - constraint)
        :precondition (and
                (not (after_sync))
                (not (after_add))
                (not (failure))
