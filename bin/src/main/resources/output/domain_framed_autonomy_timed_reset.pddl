(define (domain trace-alignment)
    ...
    :effect (and 
      ...
      (forall (?s1 - automaton_state ?s2 - automaton_state ?c - constraint)
            (when (and
                ...
                (or
                (> (- (+(current_timestamp) (wait)) (start_clock ?c) ) (max_t_condition ?s1 ?a ?s2))
                (< (- (+(current_timestamp) (wait)) (start_clock ?c) ) (min_t_condition ?s1 ?a ?s2))
                )
                ) 
  ...
  (:action add_move_automata
      ...
      :effect (and 
        ...
        (increase (current_timestamp) (wait))
        (not (after_add_check))
      ...
      (forall (?s1 - automaton_state ?s2 - automaton_state ?c - constraint)
        (when (and
          ...
          (<= (- (+(current_timestamp) (wait)) (start_clock ?c)) (max_t_condition ?s1 ?c ?s2))
          (>= (- (+(current_timestamp) (wait)) (start_clock ?c)) (min_t_condition ?s1 ?c ?s2))

        ) (and
          (assign (start_clock ?c) (+(current_timestamp) (wait)))
        ))
      )
    ...
    :effect (and 
      ...
      (forall (?s1 - automaton_state ?s2 - automaton_state ?c - constraint)
        (when (and
          ...
          (assign (start_clock ?c)(timestamp ?t1 ?t2))
        ))
      )
  ...
  (:action skip-unused
   ...
   (:action scale_up
        ...
        (exists (?s1 ?s2 - automaton_state ?a - activity ?n - automaton_name) 
            (and
                (associated ?s1 ?n)
                (cur_s_state ?s1)
                (automaton ?s1 ?a ?s2)
                ...
                ;(not (failure_state ?s2))
                (or
                (< (+(*(wait)2) current_timestamp) (+(min_t_condition ?s1 ?a ?s2) (start_clock ?n)))
                (< (+(*(wait)2) current_timestamp) (+(max_t_condition ?s1 ?a ?s2) (start_clock ?n)))
                )
    ...
    (:action increase_wait
        ...
        :precondition (and 
            ...
            (and
                ...
                (or
                (< (+(+(wait)0.1) current_timestamp) (+(min_t_condition ?s1 ?a ?s2) (start_clock ?n)))
                (< (+(+(wait)0.1) current_timestamp) (+(max_t_condition ?s1 ?a ?s2) (start_clock ?n)))
                )
    ...
    (:action reset_wait
        ...
        (exists (?s1 ?s2 - automaton_state ?a - activity ?n - automaton_name) 
          (and
            ...
            (or 
              (> (+(wait) current_timestamp) (+(min_t_condition ?s1 ?a ?s2) (start_clock ?n)))
              (> (+(wait) current_timestamp) (+(max_t_condition ?s1 ?a ?s2) (start_clock ?n)))
            )
            )
        )
        )
